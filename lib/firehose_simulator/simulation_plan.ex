defmodule FirehoseSimulator.SimulationPlan do
  ...
  @spec play_entries([Entry.t()], integer()) :: {:ok, [map()]} | {:error, term()}
  def play_entries(entries, import_offset_ms) when is_list(entries) do
    Enum.reduce_while(entries, {:ok, []}, fn entry, {:ok, acc} ->
      total_offset_ms = entry.offset_ms + import_offset_ms

      case FirehoseSimulator.play_with_offset(
             entry.scenario,
             total_offset_ms,
